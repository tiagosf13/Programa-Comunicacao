...
def encrypt (cipherkey, data):
	...
	# Encodes a binary string to text
					...
					if validate_response(dic1) == False:
						...
						if start_bool == True:
							...
							for element in dic["message"]:
								print(CYAN+element[0]+" ("+element[1]+")"+": "+"\n"+WHYTE+decrypt(find_cipher(client_sock, element[1]),element[2])+RESET+"\n")
							start_bool = False
						else:
							...
							os.system('cls' if os.name == 'nt' else 'clear')
							mensagem_encriptada = encrypt(dic1["cipher"], mensagem)
							dic = { "op": "TALK", "client_id": client_id, "message" : mensagem_encriptada }
							dic = sendrecv_dict(client_sock, dic)
...
def find_cipher(client_sock, client_id):
	dic = { "op": "CIPHER", "client_id": client_id }
	dic = sendrecv_dict(client_sock, dic)
	if validate_response(dic) == False:
...
def main():
	...
	nome_host = "localhost" # Localhost by default
